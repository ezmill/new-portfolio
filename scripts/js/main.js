...
// 
    ...
    // }, onProgress, onError);
    objLoader.load(PATH + "models/name.obj", function(object) {
        LENS = object;
        console.log(LENS);
...
function createMenu(){
    rNav = new RefractiveNavigation(scene, camera, renderer, textures);
    rNav.init();
    createMultipassMaterial();
}
function createMultipassMaterial(){
    ...
    geometry = new THREE.PlaneGeometry(renderSize.x*1.2, renderSize.y*1.2);
    ...
    // }
    // rNav.refractionPlaneMaterial.uniforms["map"].value = mMaterial.buffers[2].renderTarget;
...
function draw() {
    ...
    // for(var i = 0; i < rNav.meshes.length; i++){
        // rNav.meshes[i].visible = false;
    // }
    ...
    magnifyingGlass.cubeCamera.position.copy( new THREE.Vector3(0,0,176)   );
    // rNav.cubeCamera.position.copy( new THREE.Vector3(0,0,1500)  );
    // screensaver.cubeCamera.position.copy( new THREE.Vector3(0,0,999) );
    // rNav.cubeCamera.updateCubeMap( renderer, scene );
    magnifyingGlass.cubeCamera.updateCubeMap( renderer, scene );

    //Render the scene
    // for(var i = 0; i < rNav.meshes.length; i++){
        // rNav.meshes[i].visible = true;
    // }    
    ...
    camera.position.x = (-camera.position.x + cameraMouse.x)*0.025;
    camera.position.y = (camera.position.y - cameraMouse.y)*0.025;
