function RefractiveNavigation(SCENE, CAMERA, RENDERER, TEXTURES){
	this.scene = SCENE;
	this.camera = CAMERA;
	...
	this.init = function(){
		...
		// this.refractionPlaneGeometry = LENS.children[0].geometry;
		...
		this.refractionShader = new RefractionShader(1.0);
		...
		var circleGeometry = new THREE.SphereGeometry(50,50,50);
		circleGeometry.mergeVertices();
    	circleGeometry.computeFaceNormals();
    	circleGeometry.computeVertexNormals(true);
		for(var i = 0; i < 10; i++){
			var circle = new THREE.Mesh(circleGeometry, this.refractionPlaneMaterial);
			this.scene.add(circle);
			circle.position.y = i + (200*(-i/2));
		...
		// this.refractionPlane.scale.set(100,100,100);
		// this.refractionPlane.position.z = 1500;
		// this.refractionPlane.position.copy(LENS.children.position);
